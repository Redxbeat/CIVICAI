...
try:
    ...
    from .extraction_service import normalize_and_extract
...
def process_citizen_input(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    location = payload.get("location") or "Kochi"

    # Step 1: Gemini AI Multilingual Detection & Translation
    gemini_translation = gemini_detect_and_translate(original_text)
    language = gemini_translation.get("language") or "English"
    translated_text = gemini_translation.get("translated_text") or original_text

    # Step 2: Gemini AI Classification & Urgency Assessment
    gemini_classification = gemini_classify_and_score(translated_text)
    category = payload.get("category") or gemini_classification.get("category", "Roads")
    subcategory = gemini_classification.get("subcategory", "Road Connectivity")
    urgency = payload.get("urgency") if payload.get("urgency") is not None else gemini_classification.get("urgency", 7.5)

    # Step 3: Gemini AI Entity Extraction
